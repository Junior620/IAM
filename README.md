...
Les pages `/institut`, `/participer`, `/actualites-medias` et `/newsletter` restent en mode construction tant que leurs variables respectives `INSTITUTE_PAGE_ENABLED`, `PARTICIPATE_PAGE_ENABLED`, `NEWS_MEDIA_PAGE_ENABLED` et `NEWSLETTER_PAGE_ENABLED` ne sont pas réglées sur `true`. Pour publier une page, modifier sa variable serveur dans Vercel puis redéployer le site.
