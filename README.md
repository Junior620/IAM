...
Les pages `/institut`, `/participer` et `/actualites-medias` restent en mode construction tant que leurs variables respectives `INSTITUTE_PAGE_ENABLED`, `PARTICIPATE_PAGE_ENABLED` et `NEWS_MEDIA_PAGE_ENABLED` ne sont pas réglées sur `true`. Pour publier une page, modifier sa variable serveur dans Vercel puis redéployer le site.
